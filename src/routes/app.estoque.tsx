  ...
  return (
    <div className="space-y-5 p-4 md:p-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="font-display text-3xl tracking-wide md:text-4xl">Estoque</h1>
      ...
      <Tabs defaultValue="produtos">
        <TabsList>
          ...
          <TabsTrigger value="movimentacao">Movimentações</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="movimentacao">
          <Card className="border-border bg-card p-2 md:p-4">
            {loading ? (
              ...
              <p className="py-12 text-center text-sm text-muted-foreground">Nenhuma movimentação ainda.</p>
            ) : (
              <div className="space-y-1">
                {movements.map((m) => (
                  <div key={m.id} className="flex items-center gap-3 rounded-lg p-2.5 text-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="truncate text-xs text-muted-foreground">
                        {m.reason ?? (m.type === "in" ? "Entrada" : "Saída")} • {formatDateBR(m.created_at.slice(0, 10))}
                      </div>
                    </div>
      ...
      <Dialog open={!!moveProduct} onOpenChange={(o) => !o && setMoveProduct(null)}>
        <DialogContent className="bg-card">
          {moveProduct && (
            <MovementForm
              product={moveProduct}
              onDone={() => { setMoveProduct(null); load(); }}
            />
          )}
        </DialogContent>
      </Dialog>
...
function ProductForm({ onDone }: { onDone: () => void }) {
  ...
  return (
    <>
      <DialogHeader><DialogTitle className="font-display text-2xl tracking-wide">Novo produto</DialogTitle></DialogHeader>
      <div className="space-y-3">
        <div className="space-y-1.5"><Label>Nome *</Label><Input value={name} onChange={(e) => setName(e.target.value)} /></div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label>Categoria</Label><Input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Ex: Pomada" /></div>
          <div className="space-y-1.5"><Label>Estoque inicial</Label><Input type="number" value={stock} onChange={(e) => setStock(e.target.value)} /></div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1.5"><Label>Custo</Label><Input type="number" step="0.01" value={cost} onChange={(e) => setCost(e.target.value)} /></div>
          <div className="space-y-1.5"><Label>Venda</Label><Input type="number" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} /></div>
          <div className="space-y-1.5"><Label>Mín. alerta</Label><Input type="number" value={min} onChange={(e) => setMin(e.target.value)} /></div>
        </div>
  ...
  return (
    <>
      <DialogHeader>
        ...
        <p className="text-sm text-muted-foreground">Estoque atual: <span className="font-mono text-gold">{product.stock_quantity}</span></p>
      </DialogHeader>
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          ...
          <Button variant={type === "out" ? "default" : "outline"} onClick={() => setType("out")} className={type === "out" ? "bg-destructive text-destructive-foreground" : ""}>
            <ArrowDown className="mr-1 h-4 w-4" /> Saída
          </Button>
        </div>
        <div className="space-y-1.5"><Label>Quantidade</Label><Input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} /></div>
        <div className="space-y-1.5"><Label>Motivo</Label><Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder={type === "in" ? "Compra, reposição..." : "Uso interno, descarte..."} /></div>
        <Button onClick={submit} disabled={saving} className="w-full bg-gradient-gold text-gold-foreground hover:opacity-90">
